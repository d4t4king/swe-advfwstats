...
if ($onetime) {
	...
	foreach my $line ( @lines ) {
		...
		if ($line =~ /\bIN=(eth[0-3])\b/) { my $if = $1; $iface_pkts{$if}{$mkt}++; }
		if ($line =~ / (\.\.FFC\.\.not\.GREEN\.subnet\.\.|Denied-by-\w+:[a-zA-Z0-9]+) /) {
			my $f = $1;
			$f = trim($f);
			next if ((!defined($f)) or ($f eq ""));
			print colored("Filter not in known filters list: $f \n", "yellow") 
				if (!exists($known_filters{$f}));
			$filters{$f}{$mkt}++;
		}
		if ($line =~ / SRC=(.*) /) { 
			$src = $1; 
			...
			if ($src =~ /[\[\]\{\}\(\)]/) { $src = quotemeta($src); }
			if ($src !~ /(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}?(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/) {
				print colored("Captured source IP address doesn't look valid: $src\n", "yellow");
				next;
			}
			$sources{$src}{$mkt}++; 
		}
		if ($line =~ / DST=(.*) /) { 
			...
			if ($dst =~ /[\[\]\{\}\(\)]/) { $dst = quotemeta($dst); }
			if ($dst !~ /(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}?(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/) {
				print colored("Captured dest IP address doesn't look valid: $dst\n", "yellow");
				next;
			}
			$dests{$dst}{$mkt}++; 
		}
